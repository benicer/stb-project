...
class Page extends BasePage {
    ...
    init() {
        let source: List.ISource = {
            url: this.request.back_url || "-1"
        }
        this.source.saveToLocal(Json.serializ(source));
    }
    load() {
        if (this.cokStatus.getCookie()) var memo: Record.IMemo = Json.deSerializ(this.cokStatus.getCookie());
    ...
    openPrevious() {
        let source: Record.ISource = Json.deSerializ(this.source.takeToLocal());
...
PageRegister(Page, {
    ...
    source: new PageSource(new Cookie(`${ConfigBasic.mainCookieName}_record_source`)),
    cokStatus: new Cookie(`${ConfigBasic.mainCookieName}_record_status`)
});